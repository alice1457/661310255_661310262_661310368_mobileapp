...
class PetRecord(BaseModel):
    ...
    record_image: str | None = None  # URL รูปภาพประกอบบันทึก
...
async def create_record(record: PetRecord):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO pet_records (pet_id, record_type, title, description, record_date, record_image) VALUES (%s, %s, %s, %s, %s, %s)",
            (record.pet_id, record.record_type, record.title, record.description, record.record_date, record.record_image),
        )
        conn.commit()
...
async def update_record(record_id: int, record: PetRecord):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE pet_records SET pet_id=%s, record_type=%s, title=%s, description=%s, record_date=%s, record_image=%s WHERE id=%s",
            (record.pet_id, record.record_type, record.title, record.description, record.record_date, record.record_image, record_id),
        )
        conn.commit()
        if cursor.rowcount == 0:
            raise HTTPException(status_code=404, detail="Record not found")
...
async def update_booking_payment(booking_id: int, body: dict):
    """User: อัปโหลดสลิปหลังชำระเงิน
    body: { "payment_slip": "...", "payment_status": "paid" }
    """
    slip = body.get("payment_slip", "")
    pay_status = body.get("payment_status", "paid")
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
