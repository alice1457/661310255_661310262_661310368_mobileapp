...
def main(page: ft.Page):
    ...
    def lpad():
        return max(16, min(48, int(page.width * 0.04)))

    def sidebar_w():
        return min(300, int(page.width * 0.32))

    def max_card_w():
        if is_landscape():
            return min(640, page.width - sidebar_w() - 32)
        return page.width - 32

    # ── Responsive dialog helpers ──────────────────────────────────
    def dlg_w():
        """Max width for dialog content — fits phone screens."""
        return min(page.width - 48, 420)

    def dlg_max_h():
        """Max height for scrollable dialog content area."""
        # In landscape the keyboard + status bar eat a lot of vertical space
        ratio = 0.50 if is_landscape() else 0.62
        return max(160, int(page.height * ratio))

    def dlg_inset():
        """Reduce vertical inset in landscape so dialog doesn't overflow."""
        v = 12 if is_landscape() else 24
        return ft.padding.symmetric(horizontal=16, vertical=v)

    def scrollable_dlg_col(controls, spacing=10):
        """Column that scrolls inside a dialog — safe for portrait + landscape."""
        return ft.Column(controls, spacing=spacing, tight=True,
                         scroll=ft.ScrollMode.AUTO)

    def dlg_content(controls, spacing=10):
        """Container wrapping a scrollable column, sized for dialog use."""
        return ft.Container(
            width=dlg_w(),
            height=dlg_max_h(),
            content=scrollable_dlg_col(controls, spacing),
        )
    ...
    def show_vaccines_page(pet: dict, is_admin: bool = False):
                    ...
                    def make_edit_v(vdata):
                        def do(e):
                            ...
                            edlg = ft.AlertDialog(
                                ...
                                inset_padding=dlg_inset(),
                                title=ft.Text("แก้ไขวัคซีน", size=15, weight=ft.FontWeight.BOLD, color=DARK),
                                content=dlg_content([tf_name, tf_dose, tf_date, tf_next, tf_clinic, tf_note, st]),
                                actions=[
        ...
        page.views.append(ft.View(
            ...
            controls=[ft.Column(expand=True, spacing=0, controls=[
                ...
                ft.Container(
                    expand=True,
                    padding=ft.padding.only(left=lpad(), right=lpad(), top=16, bottom=16),
                ...
                for rec in recs:
                    ...
                    def make_edit(rec_data):
                        def do(e):
                            ...
                            tf_img = ft.TextField(
                                ...
                                hint_text="https://i.imgur.com/... หรือ imgbb.com",
                                value=rec_data.get("record_image", "") or "",
                            ...
                            def submit_edit(ev):
                                ...
                                try:
                                    res = requests.put(f"{RECORDS_URL}/{rec_data['id']}", json={
                                        ...
                                        "record_image": tf_img.value.strip() or None,
                                    }, timeout=5)
                                    if res.status_code == 200:
                                        edit_dlg.open = False
                                        page.update()
                            ...
                            edit_dlg = ft.AlertDialog(
                                ...
                                content=dlg_content([tf_type, tf_title, tf_desc, tf_date, tf_img, st]),
                                actions=[
                                    ft.TextButton("ยกเลิก", style=ft.ButtonStyle(color=LIGHT),
                    ...
                    def make_detail(rd):
                        def show_detail(e):
                            ...
                            img_url = rd.get("record_image", "") or ""
                            img_widget = ft.Container(
                                border_radius=10,
                                clip_behavior=ft.ClipBehavior.ANTI_ALIAS,
                                content=ft.Image(
                                    src=img_url,
                                    width=min(page.width - 80, 280),
                                    height=min(180, int(page.height * 0.22)),
                                    fit="contain",
                                    border_radius=10,
                                    error_content=ft.Container(
                                        height=40,
                                        content=ft.Text("โหลดรูปไม่ได้", size=10, color=LIGHT),
                                        alignment=ft.alignment.Alignment(0, 0),
                                    ),
                                ),
                            ) if img_url.strip() else ft.Container(height=0)
                            ...
                            detail_dlg = ft.AlertDialog(
                                ...
                                content=ft.Container(
                                    ...
                                    content=ft.Column([
                                        ...
                                        ft.Container(height=8) if img_url else ft.Container(height=0),
                                        img_widget,
                            ...
                            content=ft.Row([
                                ...
                                ft.Column([
                                    ...
                                    ft.Row([
                                        ...
                                        *(
                                            [ft.Icon(ft.Icons.IMAGE_OUTLINED, color=LIGHT, size=12)]
                                            if rec.get("record_image") else []
                                        ),
                                    ], spacing=4),
    ...
    def show_pet_detail(pet: dict, idx: int):
        ...
        def open_edit(e):
            ...
            dlg = ft.AlertDialog(
                ...
                content=dlg_content([tf_name, tf_type, tf_age, tf_img, tf_vaccine, st]),
                ...
                ft.Container(
                    ...
                    content=ft.Column(
                        ...
                        controls=[
                            ft.Container(
                                ...
                                content=ft.Row(
                                    [
                                        ...
                                        ft.Container(
                                            width=sidebar_w() if is_landscape() else None,
                                            expand=not is_landscape(),
                                            content=ft.Container(
                                        ...
                                        # ── RIGHT (landscape only): records card ──
                                        *(
                                            [
                                                ft.Container(width=16),
                                                ft.Container(
                            ...
                            # ── portrait only: records card below ──
                            *(
                                [
                                    ft.Container(
                                        margin=ft.margin.only(left=lpad(), right=lpad(), bottom=16),
                                        padding=ft.padding.all(16),
    ...
    def show_payment_page(booking_id: int, pet: dict, check_in: str, check_out: str, total_price: float):
        ...
        confirm_btn = btn("✅ ยืนยันการชำระเงิน", submit_payment, height=50, radius=12, expand=True)

        if is_landscape():
            body_content = ft.Row([
                ft.Container(
                    expand=True,
                    content=ft.Column([summary_card], scroll=ft.ScrollMode.AUTO, expand=True, spacing=0),
                ),
                ft.Container(width=16),
                ft.Container(
                    expand=True,
                    content=ft.Column([payment_card, ft.Container(height=12), confirm_btn],
                                      scroll=ft.ScrollMode.AUTO, expand=True, spacing=0),
                ),
            ], vertical_alignment=ft.CrossAxisAlignment.START, spacing=0, expand=True)
        else:
            body_content = ft.Column([summary_card, payment_card, ft.Container(height=12), confirm_btn],
        ...
        page.views.append(ft.View(
            ...
            controls=[ft.Column(expand=True, spacing=0, controls=[
                ...
                ft.Container(
                    expand=True,
                    padding=ft.padding.only(left=lpad(), right=lpad(), top=20, bottom=20),
    ...
    def show_booking_page(pet: dict):
        ...
        if is_landscape():
            inner = ft.Row([
                ft.Container(expand=True, content=booking_card),
                ft.Container(width=16),
                ft.Container(
                    width=sidebar_w(),
                    content=ft.Column([
            ...
            inner = ft.Column([booking_card, btn("ถัดไป → ชำระเงิน", submit_booking, height=50, radius=12, expand=True)],
                              spacing=16, scroll=ft.ScrollMode.AUTO, expand=True)
        ...
        page.views.append(ft.View(
            ...
            controls=[ft.Column(expand=True, spacing=0, controls=[
                ...
                ft.Container(
                    expand=True,
                    padding=ft.padding.only(left=lpad(), right=lpad(), top=20, bottom=20),
                    content=inner if is_landscape() else ft.Column([inner], expand=True, spacing=0),
                ),
            ])],
        ))
        page.update()
    ...
    def load_all_pets():
        ...
        try:
            ...
            priority = {"staying": 3, "approved": 2, "pending": 1}
            for b in all_books:
                pid_b = b.get("pet_id")
    ...
    def show_profile_page(e):
        ...
        def open_edit_profile(e):
            ...
            dlg = ft.AlertDialog(
                ...
                content=dlg_content([tf_name, tf_email, tf_phone, tf_password, status_txt]),
                actions=[ft.TextButton("ยกเลิก", style=ft.ButtonStyle(color=LIGHT),
                                       on_click=lambda e: (setattr(dlg, "open", False), page.update())),
        ...
        avatar_card = ft.Container(
            bgcolor=DARK,
            border_radius=16 if is_landscape() else 0,
        ...
        if is_landscape():
            profile_body = ft.Row([
                ft.Container(
                    width=sidebar_w(),
                    content=ft.Column([avatar_card], spacing=0),
                ),
                ft.Container(width=16),
                ft.Container(
                    expand=True,
                    content=ft.Column(
                        [info_card, ft.Container(height=12), menu_card],
                        scroll=ft.ScrollMode.AUTO, spacing=0,
                    ),
                ),
            ], expand=True, spacing=0, vertical_alignment=ft.CrossAxisAlignment.START)
        else:
            profile_body = ft.Column([
                avatar_card,
                ft.Container(height=16),
                ft.Container(margin=ft.margin.symmetric(horizontal=16), content=info_card),
                ft.Container(height=12),
                ft.Container(margin=ft.margin.symmetric(horizontal=16), content=menu_card),
        ...
        page.views.append(ft.View(
            ...
            controls=[ft.Column(expand=True, spacing=0, controls=[
                ...
                ft.Container(
                    expand=True,
                    padding=ft.padding.only(
                        left=lpad() if is_landscape() else 0,
                        right=lpad() if is_landscape() else 0,
                        top=16 if is_landscape() else 0,
                        bottom=16,
                    ),
    ...
    def build_home_body():
        ...
        # landscape: hero ย่อ, pets แสดง 2 grid
        if land:
            body = ft.Row([
                # ── left sidebar ──
                ft.Container(
                    ...
                    content=ft.Column([
                        ft.Container(
                            padding=ft.padding.all(20),
                            content=ft.Column([
                                ft.Row([ft.Text("🐾", size=20), ft.Container(width=6),
                                        ft.Text("PET HOTEL", size=16, weight=ft.FontWeight.BOLD, color="#FFF")]),
                                ft.Container(height=4),
                                ft.Text("Your Pet Dashboard", size=10, color=LIGHT),
                                ...
                                ft.Container(
                                    ...
                                    padding=ft.padding.symmetric(horizontal=10, vertical=5),
                                    content=ft.Text("🐾 จุ้มปุ๊คโฮเทล", size=11, color="#000", weight=ft.FontWeight.BOLD),
                                ),
                                ft.Container(height=16),
                                ft.Text("ยินดีตอนรับสู่\nระบบจัดการโรงแรมสัตว์เลี้ยง",
                                        size=14, weight=ft.FontWeight.BOLD, color="#FFF"),
                                ft.Container(height=24),
                                *(
                                    [
                                        btn("📋 การจองทั้งหมด", show_admin_bookings_page,
                                            height=38, radius=8, bgcolor="#2A2A2A", text_color="#FFF", expand=True),
                                        ft.Container(height=8),
                                        btn("👥 ผู้ใช้", show_admin_users_page,
                                            height=38, radius=8, bgcolor="#2A2A2A", text_color="#FFF", expand=True),
                                    ] if is_admin else []
                                ),
                            ], spacing=0),
                        ...
                        ft.GestureDetector(
                            ...
                            content=ft.Container(
                                ...
                                content=ft.Row([
                                    ft.Container(width=36, height=36, border_radius=18, bgcolor=PRIMARY,
                                                 alignment=ft.alignment.Alignment(0, 0),
                                                 content=ft.Icon(ft.Icons.PERSON, color=DARK, size=20)),
                                    ft.Container(width=10),
                                    ft.Column([user_name_txt, user_email_txt], spacing=2, expand=True),
                                    ft.Icon(ft.Icons.CHEVRON_RIGHT, color=LIGHT, size=16),
                                ], vertical_alignment=ft.CrossAxisAlignment.CENTER),
                            ),
                        ),
    ...
    def build_login_view():
        ...
        login_form = ft.Column(scroll=ft.ScrollMode.AUTO, expand=True, spacing=0, controls=[
            ft.Container(
        ...
        if is_landscape():
            main_area = ft.Row([
                ft.Container(expand=True),
                ft.Container(
                    width=min(420, page.width * 0.55),
                    content=login_form,
                ),
                ft.Container(expand=True),
            ], expand=True, spacing=0)
        else:
            main_area = login_form
